...
class BasicCommaSeparatedUserField(CharField):
    ...
    default_error_messages = {
        'unknown': _(
            "Some usernames are unknown or no longer "
            "active: {users}."
        ),
        'max': _(
            "Ensure this value has at most {limit_value} "
            "distinct items (it has {show_value})."
        ),
        'min': _(
        ...
        'filtered': _(
            "Some usernames are rejected: {users}."
        ),
        'filtered_user': _(
            "{username}"
        ),
        'filtered_user_with_reason': _(
            "{username} ({reason})"
        ),
    }
    ...
    def to_python(self, value):
        """Normalize data to an unordered list of distinct, non empty,
        whitespace-stripped strings."""
        value = super().to_python(value)
        if value in EMPTY_VALUES:
            return []
        return list(set([name.strip(
        ) for name in value.split(',') if name and not name.isspace()]))
    ...
    def validate(self, value):
        ...
        if self.max and count > self.max:
            raise ValidationError(self.error_messages['max'].format(
                limit_value=self.max, show_value=count))
        if self.min and count < self.min:
            raise ValidationError(self.error_messages['min'].format(
    ...
    def clean(self, value):
        ...
        users = list(user_model.objects.filter(is_active=True,
                     **{'{0}__in'.format(name_user_as): names}))
...
# the minimum to declare to enable the feature
arg_default = d.get('arg_default')
...
if app_name in settings.INSTALLED_APPS and arg_default:
    ...
    autocompleter_app['version'] = getattr(
        import_module(app_name), '__version__', None)
    # does something like "from ajax_select.fields import AutoCompleteField"
    auto_complete_field = getattr(
        import_module(app_name + '.fields'), field_name)

    class CommaSeparatedUserField(
            BasicCommaSeparatedUserField, auto_complete_field):
        ...
        def set_arg(self, value):
            """Same as it is done in ajax_select.fields.py
