...
notification = None
notifier_app_id = getattr(
    settings, 'POSTMAN_NOTIFIER_APP', 'pinax_notifications')
if notifier_app_id:
    try:  # priority to an app label
        ...
        # "None if the application doesn’t contain a models module"
        notification = notifier_app_config.models_module
...
if name and name in settings.INSTALLED_APPS:
    ...
    # the app didn't adjust to the signature change in Django 1.7 (last check: v1.2.2)
    if name == 'mailer':
...
DISABLE_USER_EMAILING = getattr(
    settings, 'POSTMAN_DISABLE_USER_EMAILING', False)
# custom default 'from'
FROM_EMAIL = getattr(settings, 'POSTMAN_FROM_EMAIL',
...
def format_body(sender, body, indent=_("> "), width=WRAP_WIDTH):
    ...
    """
    indent = force_str(
        indent)  # join() doesn't work on lists with lazy translation objects ; nor startswith()
    wrapper = TextWrapper(
        width=width, initial_indent=indent, subsequent_indent=indent)
    # rem: TextWrapper doesn't add the indent on an empty text
    quote = '\n'.join([line.startswith(indent) and indent +
...
def email(subject_template, message_template_name, recipient_list, object, action, site):
    ...
    try:
        html_message = render_to_string(
            message_template_name + '.html', context)
    ...
    try:
        ...
        if message == '':
            raise TemplateDoesNotExist(
                "The .txt template can't be empty when the .html template doesn't exist")
    ...
    # expected syntax: 'myapp.mymodule.myfunc'
    elif isinstance(FROM_EMAIL, str) and not '@' in FROM_EMAIL:
    ...
    send_mail(subject, message, from_email, recipient_list,
              fail_silently=True, html_message=html_message, **kwargs)


def email_visitor(object, action, site):
    """Email a visitor."""
    email('postman/email_visitor_subject.txt', 'postman/email_visitor',
          [object.email], object, action, site)
...
def notify_user(object, action, site):
    ...
    if action == 'rejection':
        ...
        label = 'postman_reply' if (
            parent and parent.sender_id == object.recipient_id) else 'postman_message'
    else:
        return
    if notification:
        ...
        if not DISABLE_USER_EMAILING and email_address and user.is_active:
            email('postman/email_user_subject.txt', 'postman/email_user',
                  [email_address], object, action, site)
